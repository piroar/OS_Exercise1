...
int main(){
    ...
    while (running)
    {
        printf("Activate Proccess A with: BEGIN A\n");
	    fgets(input, BUFSIZ, stdin);
        if (strncmp(input,"BEGIN A",7)==0)
        {
            running=0;
        }
...
void *Input_Thread(struct shared_use_st *shared){
    ...
    while (running)
    {
        if (shared->flab==0)
        {
            ...
            strncpy(shared->A_B, buffin, TEXT_SZ);
...
void *Output_Thread(struct shared_use_st *shared){
    ...
    while (running)
    {
        if (shared->flba == 1)
        {
            strncpy(buffout,shared->B_A, TEXT_SZ);
            printf("From B:%s\n",buffout);
